...
export default function MainPage() {
  ...
  const [categoriesPopular, setCategoriesPopular] = useState<Category[] | null>(
    null
  );
    ...
    <div>
      ...
      {categoriesPopular ? (
        categoriesPopular.map((categoryPopular) => (
          <div className="divCreateCard">
            <Link
              key={categoryPopular.id}
              to={`/category/${categoryPopular.name}`}
            >
              <h2>{categoryPopular.name}</h2>
              <button>
                <img src={categoryPopular.image} width={100} />
              </button>
            </Link>
          </div>
        ))
